# -*- coding: utf-8 -*-
# Generated by Django 1.11.4 on 2018-02-16 07:32
from __future__ import unicode_literals

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('professionals', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='Milestone',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('status', models.CharField(choices=[('W', 'Waiting'), ('PTH', 'Paid to Hoome'), ('PTP', 'Paid to Professional'), ('D', 'Done'), ('M', 'Payment Request')], default='W', max_length=63)),
                ('amount', models.PositiveIntegerField()),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('uuid', models.CharField(default='0', max_length=36)),
            ],
        ),
        migrations.CreateModel(
            name='Project',
            fields=[
                ('project_id', models.BigAutoField(primary_key=True, serialize=False)),
                ('project_name', models.CharField(max_length=100)),
                ('first_name', models.CharField(blank=True, max_length=64)),
                ('last_name', models.CharField(blank=True, max_length=64)),
                ('bus_name', models.CharField(blank=True, max_length=255)),
                ('project_type', models.CharField(blank=True, choices=[('R', 'REMODEL'), ('N', 'NEW BUILT HOUSE')], default='R', max_length=5)),
                ('street_address', models.TextField(blank=True)),
                ('street_address2', models.TextField(blank=True)),
                ('county', models.CharField(blank=True, max_length=64)),
                ('state', models.CharField(blank=True, max_length=64)),
                ('zipcode', models.CharField(blank=True, max_length=10)),
                ('project_cost', models.IntegerField(default=0)),
                ('contract_price', models.IntegerField(default=0)),
                ('start_date', models.DateField(auto_now_add=True)),
                ('end_date', models.DateField(auto_now_add=True)),
                ('project_description', models.TextField(blank=True)),
                ('project_status', models.CharField(default='W', max_length=1)),
                ('project_action', models.TextField(blank=True)),
                ('uuid', models.CharField(default='0', max_length=36)),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('created_by', models.CharField(choices=[('PROFESSIONAL', 'Professional'), ('CONSUMER', 'Consumer')], default='CONSUMER', max_length=16)),
                ('professional', models.ForeignKey(null=True, on_delete=django.db.models.deletion.DO_NOTHING, to='professionals.Professional')),
            ],
        ),
        migrations.CreateModel(
            name='ProjectAttachment',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('attachment_type', models.CharField(max_length=100)),
                ('project_attachment', models.FileField(upload_to='projects/attachments/%Y/%m/%d')),
                ('uploaded_at', models.DateTimeField(auto_now_add=True)),
                ('title', models.CharField(max_length=255)),
                ('project', models.ForeignKey(on_delete=django.db.models.deletion.DO_NOTHING, to='projects.Project')),
            ],
        ),
        migrations.CreateModel(
            name='ProjectPhoto',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('project_photo', models.ImageField(upload_to='projects/photos/%Y/%m/%d')),
                ('uploaded_at', models.DateTimeField(auto_now_add=True)),
                ('title', models.CharField(blank=True, max_length=255)),
                ('project', models.ForeignKey(on_delete=django.db.models.deletion.DO_NOTHING, to='projects.Project')),
            ],
        ),
    ]
